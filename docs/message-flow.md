...
## 3. Ключевые переменные и их отличия

### `isFirstMessage`
```typescript
const isFirstMessage = messageCount === 0
```
- `messageCount` берётся из `conversations.messageCount` (БД)
- Инкрементируется **в конце** `runPipeline` → всегда актуален для следующего вызова
- После `/api/admin/tools/clear-memory`: `messageCount = 0` → `isFirstMessage = true`

### `messageCount`
```typescript
const ctx = await getConversationContext(conversationId)
messageCount = ctx.messageCount  // из conversations.messageCount
```

### `history`
```typescript
const ctx = await getConversationContext(conversationId)
history = ctx.history  // последние MAX_HISTORY (6) сообщений, формат "user: ...\nassistant: ..."
```

### `lastBotMessage`
```typescript
const lastBotMessage = getLastBotMessage(history)
// последняя строка начинающаяся с "assistant:" — передаётся в классификатор интента
```
...
## 5. Синтез ответа (`synthesizeFinalAnswer`)

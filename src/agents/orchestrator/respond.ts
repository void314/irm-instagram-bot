...
export async function craftResponse(
    ...
    if (askForName) {
        extra.push('Ты ещё не знаешь имя пациента. В конце ответа вежливо спроси, как к нему обращаться.')
    }
    ...
    const systemPrompt = [
        ...
        `Сегодня: ${formatToday()}`,
        '',
        'История диалога (используй её, чтобы понять контекст и НЕ ПОВТОРЯТЬ то, что уже сказано пользователю):',
        history || 'нет',
        '',

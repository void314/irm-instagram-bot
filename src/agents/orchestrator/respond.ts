...
export async function craftResponse(
    ...
    const extra: string[] = []
    if (patientName) {
        extra.push(`Имя пациента: ${patientName}. Используй его в ответе естественно, если это уместно, не форсированно.`)
    }
    if (askForName) {
        if (suggestedName) {
            extra.push(
                `Ты ещё не знаешь подтверждённое имя пациента. ОБЯЗАЛЬНО: В конце ответа вежливо уточни, можно ли обращаться к нему как "${suggestedName}". Если нет — спроси, как правильно обращаться.`
            )
        } else {
            extra.push('Ты ещё не знаешь имя пациента. ОБЯЗАЛЬНО: В конце ответа вежливо спроси, как к нему обращаться.')
        }
    }
    if (shouldSuggestBooking) {
        extra.push('В конце ответа мягко предложи записаться на консультацию. Сделай это одним естественным предложением.')
    }
    if (shouldNudgeBooking) {
        extra.push('Пациент уже задал несколько вопросов. В конце ответа мягко напомни о возможности записаться на консультацию к врачу.')
    }
    ...
    const systemPrompt = [
        ...
        'а материал для тебя. Сформируй из него ОДИН связный ответ от своего лица на ' + LANG_LABEL[lang] + ' языке, соблюдая все правила выше.',
        'КРИТИЧЕСКИ ВАЖНО:',
        '- Если пользователь просто отвечает на твой предыдущий вопрос (например, называет имя), а факты говорят об отсутствии информации — проигнорируй такие факты и отвечай естественно по ходу диалога.',

...
import { responseFeedback, kbSuggestions, learningDocs, learnChunks } from '../../db/schema'
...
export async function processCorrection(feedbackId: bigint) {
    const feedback = await db.select().from(responseFeedback).where(eq(responseFeedback.id, feedbackId)).then(r => r[0])
    if (!feedback) return
    ...
    const topicResponse = await chat([
        { role: 'system', content: 'Выдели основную тему этого факта в 2-3 словах. Например: "Цены лазерная эпиляция", "График работы Алматы". Только тема, без кавычек.' },
...
export async function generateKbSuggestions() {
    ...
    for (const [topic, items] of Object.entries(groups)) {
        ...
        const suggestionIds = await db.insert(kbSuggestions).values({
...
export async function applySuggestion(suggestionId: bigint) {
    const suggestion = await db.select().from(kbSuggestions).where(eq(kbSuggestions.id, suggestionId)).then(r => r[0])
    if (!suggestion || suggestion.status !== 'approved') return

    // 1. Create document
    const [doc] = await db.insert(learningDocs).values({
    ...
    if (suggestion.sourceFeedbackIds && suggestion.sourceFeedbackIds.length > 0) {
        await db.update(responseFeedback)
            .set({ status: 'applied' })
            .where(inArray(responseFeedback.id, suggestion.sourceFeedbackIds.map(id => BigInt(id))))
    }
